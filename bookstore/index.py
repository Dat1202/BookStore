...
# from scapy.tools.generate_ethertypes import data
...
def user_signin():
    ...
    if request.method.__eq__('POST'):
        ...
        user_role_manage = utils.check_login(username=username,
                                  password=password,
                                  role=UserRole.INVENT_MANAGE)
        user_role_admin = utils.check_login(username=username,
                                 password=password,
                                 role=UserRole.ADMIN)
        user_role_user = utils.check_login(username=username, password=password)
        if user_role_user:
            login_user(user=user_role_user)
            url_next = request.args.get('next')
            return redirect(url_next if url_next else '/')
        elif user_role_admin:
            login_user(user=user_role_admin)
            return redirect('/admin')

        elif user_role_manage:
            login_user(user=user_role_manage)
